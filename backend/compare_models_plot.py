...
def plot_all_to_single_file(output_path: str = 'compare_models_overview.png'):
    ...
    for ax, (key, title, ylabel) in zip(axes, metrics):
        ...
        # y-limit: cap at >=1.0 for prob-like metrics; allow >1 for separation ratio
        ylim_top = max(1.0 if key != 'separation' else max(values) * 1.15, max(values) * 1.1)
...
def main():
    # Save a single image containing all six comparisons
