...
BASELINE_DBSCAN_MODELS_DIR = "baseline-dbscan-models"
BASELINE_DBSCAN_DATA_DIR = "baseline-dbscan-data"
...
def get_baseline_dbscan_models_dir():
    """Get the baseline DBSCAN models directory"""
    base_dir = get_base_models_dir()
    models_dir = os.path.join(base_dir, BASELINE_DBSCAN_MODELS_DIR)
    if not os.path.exists(models_dir):
        os.makedirs(models_dir)
    return models_dir


def get_baseline_dbscan_data_dir():
    """Get the baseline DBSCAN data directory"""
    base_dir = get_base_models_dir()
    data_dir = os.path.join(base_dir, BASELINE_DBSCAN_DATA_DIR)
...
def run_baseline_dbscan(svd_components: int = 200, eps: float = 0.1, min_samples: int = 10, random_state: int = 42):
    ...
    baseline_dbscan_models_dir = get_baseline_dbscan_models_dir()
    baseline_dbscan_data_dir = get_baseline_dbscan_data_dir()
    
    joblib.dump(centroids, os.path.join(baseline_dbscan_models_dir, 'baseline_dbscan_centroids.pkl'))
    joblib.dump(svd, os.path.join(baseline_dbscan_models_dir, 'baseline_dbscan_svd.pkl'))
    joblib.dump(
        {
            'threshold': best_th,
        ...
        os.path.join(baseline_dbscan_models_dir, 'baseline_dbscan_threshold.pkl')
    )
    train_df.to_csv(os.path.join(baseline_dbscan_data_dir, 'baseline_dbscan_train.csv'), index=False)
    test_df.to_csv(os.path.join(baseline_dbscan_data_dir, 'baseline_dbscan_test.csv'), index=False)
    print(f'✓ Saved: {os.path.join(baseline_dbscan_models_dir, "baseline_dbscan_centroids.pkl")}')
    print(f'✓ Saved: {os.path.join(baseline_dbscan_models_dir, "baseline_dbscan_svd.pkl")}')
    print(f'✓ Saved: {os.path.join(baseline_dbscan_models_dir, "baseline_dbscan_threshold.pkl")}')
    print(f'✓ Saved: {os.path.join(baseline_dbscan_data_dir, "baseline_dbscan_train.csv")}')
    print(f'✓ Saved: {os.path.join(baseline_dbscan_data_dir, "baseline_dbscan_test.csv")}')

    # Evaluation report
    print('Running evaluation...')
    reports_dir = get_evaluation_reports_dir()
    report_path = os.path.join(reports_dir, 'baseline_dbscan_evaluation_report.json')
    run_full_evaluation(
