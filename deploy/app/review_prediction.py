...
Adapted for deployment with proper path handling
"""
import os
import re
import numpy as np
...
# === Dynamically locate models folder ===
# In deployment, models are in the parent directory's models folder
...
enhanced_scaler = joblib.load(os.path.join(MODELS_DIR, "enhanced_feature_scaler.pkl"))
svd = joblib.load(os.path.join(MODELS_DIR, "best_run_svd.pkl"))
centroids = joblib.load(os.path.join(MODELS_DIR, "best_run_centroids.pkl"))
threshold_data = joblib.load(os.path.join(MODELS_DIR, "best_run_threshold.pkl"))
